...
//    long saveTime = System.currentTimeMillis() - saveFile.lastModified();
     ...
     * @param netUrlorPath  网络路径标志
     * @return
     * @throws IOException  代码中可能出现的异常操作
     */
...
//            throw new StructException("需要的文件不存在！");
     ...
     * 这里是将网络请求的结果写入到本地文件中
     *
     * @param netUrlorPath     想要保存的文件的路径通过下面代码获得
     *                 String md5Url = Md5Util.md5(vo.getBaseUrl());
     *                 String filePath = new File(vo.getmContext().getCacheDir(),
     *                 URLEncoder.encode(md5Url) + ".json").getAbsolutePath();
     * @param content  要保存的文件的具体内容
     * @throws IOException
     */
...
//            File fileBySDSon=new File(fileBySD,"AA");
            if(fileBySDSon.exists()){
                String md5Url = EncryptUtil.getStringMD5(netUrlorPath);
