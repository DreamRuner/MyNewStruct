package com.seowang.mynewstruct.net.download;

import com.seowang.mynewstruct.net.thread.MyThreadFactory;
import com.seowang.mynewstruct.net.utils.EncryptUtil;
import com.seowang.mynewstruct.net.utils.PackageUtil;
import com.seowang.mynewstruct.net.utils.StorageUtil;
...
public class DownLoadUtil {
    ...
    private static void doHttp(final String netUrl) throws IOException {
        System.out.println("DownLoad::资源的下载连接："+netUrl);
        HttpURLConnection conn= (HttpURLConnection) getUrlConnection(netUrl);
        //获取网络中需要下载的资源的大小
        if(null==conn){

            System.out.println("DownLoad::HttpUrlConnection对象为Null");
        }
        int netFileLength= (int) doDownLoadByHttp(conn);
        if(netFileLength<=0){
            System.out.println("DownLoad::长度为0000000");
            return;
        }
        //在本地创建一个同名称同大小的文件
...
//                File fileByCacheSon=new File(fileByCache,"AA");
        ...
        String md5Url = EncryptUtil.getStringMD5(netUrl);
        //以图片的网络连接作为对应的文件名称，进行保存图片
        final String saveHopePath=new File(fileByCacheSon, URLEncoder.encode(md5Url)).getAbsolutePath();
//        final String saveHopePath=new File(fileByCacheSon, netUrl).getAbsolutePath();
        System.out.println("DownLoad::文件存储的目录：：" + saveHopePath);
        RandomAccessFile raf = new RandomAccessFile(saveHopePath, "rw");
        raf.setLength(netFileLength);
        byteSize=netFileLength/threadNum;
        System.out.println("DownLoad::文件大小：" + netFileLength);
        System.out.println("DownLoad::线程个数："+threadNum);
        System.out.println("DownLoad::子区域大小："+byteSize);
        System.out.println("DownLoad::--------开启线程下载----------------------------------");
        ...
        for(int workId=0;workId<threadNum;workId++){
            ...
            System.out.println("DownLoad::这是第"+workId+"个线程操作");
            System.out.println("DownLoad::起始位置：" + startByte + "-------" + "结束位置：" + endByte);
                    ...
                    try {
                        ...
                        MyThreadFactory.getThreadPoolExecutor().execute(new Runnable() {
                            ...
                            public void run() {
                                ...
                                try {
                                    System.out.println("DownLoad::Http进行第"+ finalWorkId +"个任务在下载！！");
                                    ...
                                    while ((len = is.read(buffer)) != -1) {
                                        ...
                                        System.out.println("DownLoad::Http进行第" + finalWorkId + "个任务在下载！的长度"+len);
                                    }
        ...
        System.out.println("DownLoad::Http已经下载完成！！");
    }
    /**
     * 对于Https连接形式下的数据下载操作
     *
    ...
    private static void doHttps(final String netUrl) throws IOException {
                    ...
                    try {
                        ...
                        MyThreadFactory.getThreadPoolExecutor().execute(new Runnable() {
                            ...
                            public void run() {
                                RandomAccessFile raf = null;
                                try {
                                    System.out.println("DownLoad::Https进行第"+ finalWorkId +"个任务在下载！！");
        ...
        System.out.println("DownLoad::Https已经下载完成！！");
    }
